...
def pose_garment(
        ...
        pose2rot: bool = True,
        A_POSE: Tensor = None,
        A_joint_transforms: Tensor = None
):
    """
    ...
    # Add shape contribution
    # v_shaped = v_template + blend_shapes(betas, shapedirs)
    # TODO: GEN: the shape of our template in bedlam is fixed. don't reshape template clothing!
    v_shaped = v_template

    # A = joint_transforms
    A = torch.matmul(joint_transforms, torch.inverse(A_joint_transforms))
    ...
    if pose2rot:
        ...
        # calc A-pose transformation and reduce pose_offsets
        rot_mats_A = batch_rodrigues(A_POSE.view(-1, 3)).view([batch_size, -1, 3, 3])
        pose_feature_A = (rot_mats_A[:, 1:, :, :] - ident).view([batch_size, -1])
        pose_offsets_A = torch.matmul(pose_feature_A, posedirs).view(batch_size, -1, 3)
    else:
        pdb.set_trace()
    ...
    v_posed = pose_offsets + v_shaped - pose_offsets_A
    ...
    T = torch.matmul(W, A.reshape(batch_size, num_joints, 16)) \
