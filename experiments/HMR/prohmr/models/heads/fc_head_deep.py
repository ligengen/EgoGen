...
class FCHeadDeep(nn.Module):
    ...
    def __init__(self, cfg: CfgNode, fc_head_inchannel=None):
        ...
        """
        super(FCHeadDeep, self).__init__()
        self.cfg = cfg
        self.npose = 6 * (cfg.SMPL.NUM_BODY_JOINTS + 1)
        self.layers = nn.Sequential(nn.Linear(fc_head_inchannel,cfg.MODEL.FC_HEAD.NUM_FEATURES),
                                    nn.ReLU(inplace=False),
                                    nn.Linear(cfg.MODEL.FC_HEAD.NUM_FEATURES, 256),
                                    nn.ReLU(inplace=False),
                                    nn.Linear(256, 128),
                                    nn.ReLU(inplace=False),
                                    nn.Linear(128, 13)
                                    )
        # nn.init.xavier_uniform_(self.layers[2].weight, gain=0.02) todo?
