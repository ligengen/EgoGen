...
"""
eval trained model on crowd behaviors
"""
import argparse
import datetime
import os
...
from crowd_ppo.dummy_vector_env import DummyCrowdVectorEnv
from crowd_ppo.crowd_env_crowd_eval import CrowdEnv
from crowd_ppo.primitive_model import load_model
from crowd_ppo.ppo_policy import GAMMAPPOPolicy
# from exp_GAMMAPrimitive.utils.environments import BatchGeneratorSceneTrain as SceneTrainGen
from exp_GAMMAPrimitive.utils.environments import CrowdMotion as SceneTrainGen
...
def main():
    test_envs = DummyCrowdVectorEnv([lambda: CrowdEnv(init_env), lambda: CrowdEnv(init_env2), lambda: CrowdEnv(init_env3), lambda: CrowdEnv(init_env4)])
...
if __name__ == "__main__":
    ...
    parser.add_argument("--repeat-per-collect", type=int, default=3) # max_train_iter_1/2f
    parser.add_argument("--batch-size", type=int, default=256)
    parser.add_argument("--training-num", type=int, default=1)
    parser.add_argument("--test-num", type=int, default=num_agents)
    ...
    parser.add_argument("--weight-kld", type=float, default=10.0) # gamma: 10
    ...
    parser.add_argument("--logdir", type=str, default="./log/log_eval_crowd_motion")
    parser.add_argument("--render", type=float, default=0.)
    parser.add_argument(
        "--device", type=str, default="cuda" if torch.cuda.is_available() else "cpu"
    )
    parser.add_argument("--resume-path", type=str, default="data/checkpoint_best.pth")
    ...
    cfg, \
    genop_2frame_male, \
    genop_2frame_female = load_model(crowd=True)
    # dataset
    ...
    scene_sampler = SceneTrainGen(dataset_path='', motion_seed_list=motion_seed_list, scene_dir='data/scenes', # scene_list=scene_list,
    ...
    max_steps = cfg.trainconfig.max_depth

    pts = np.zeros((4, 3))
    r = 2
    s = np.random.rand()
    t = np.linspace(s, s+2*np.pi, 4, endpoint=False)
    x = r * np.cos(t)
    y = r * np.sin(t)
    pts[:,:2] = np.c_[x, y]
    start_target = [(pts[0], pts[2]), (pts[1], pts[3]), (pts[2], pts[0]), (pts[3], pts[1])]

    body_scene_data, body_scene_data2, body_scene_data3, body_scene_data4, = scene_sampler.next_body(start_target=start_target, fixed_seed=True, num_agents=num_agents)
    # name = str(dimos_res[idx]).split("/")[8]
    name = "crowd4_"
    init_env = [cfg, genop_2frame_male, genop_2frame_female, \
                bm_path, body_scene_data, smplxparser_1frame, smplxparser_2frame, \
                smplxparser_mp, feet_marker_idx, body_markers, vposer, '0', name]
    init_env2 = [cfg, genop_2frame_male, genop_2frame_female, \
                bm_path, body_scene_data2, smplxparser_1frame, smplxparser_2frame,\
                smplxparser_mp, feet_marker_idx, body_markers, vposer, '1', name]
    init_env3 = [cfg, genop_2frame_male, genop_2frame_female, \
                bm_path, body_scene_data3, smplxparser_1frame, smplxparser_2frame,\
                smplxparser_mp, feet_marker_idx, body_markers, vposer, '2', name]
    init_env4 = [cfg, genop_2frame_male, genop_2frame_female, \
                bm_path, body_scene_data4, smplxparser_1frame, smplxparser_2frame,\
                smplxparser_mp, feet_marker_idx, body_markers, vposer, '3', name]
    main()
