...
from crowd_ppo.crowd_env_2f import CrowdEnv
...
from models.models_policy_ppo_gru import GAMMAActor, GAMMACritic, GAMMAPolicyBase
from models.models_policy_ppo_gru import ActorCritic
...
def get_args():
    ...
    # parser.add_argument("--logdir", type=str, default="/mnt/vlg-nfs/genli/log_pretrain_dep13_seedori/log_2f_ego_gru_rpene1_rlook0.3")
    parser.add_argument("--logdir", type=str, default="/mnt/scratch/genli/post_cvpr_solving_look_up/log_2f_shorter_distance_finetune")
    parser.add_argument("--render", type=float, default=0.)
    parser.add_argument(
        "--device", type=str, default="cuda" if torch.cuda.is_available() else "cpu"
    )
    parser.add_argument("--resume-path", type=str, default='/mnt/scratch/genli/post_cvpr_solving_look_up/log_2f_shorter_distance/collision-avoidance/ppo/0/240410-105725/checkpoint_113.pth')
    parser.add_argument("--resume-buffer", type=str, default=None)
    parser.add_argument("--resume-id", type=str, default=None)
    parser.add_argument("--finetune", default=True, action="store_false")
    parser.add_argument("--deterministic-eval", default=False, action="store_true")
    parser.add_argument(
        "--logger",
        type=str,
        default="tensorboard",
...
def main(args=get_args()):
    env = CrowdEnv(init_env, finetuning=args.finetune)
    train_envs = DummyVectorEnv([lambda: CrowdEnv(init_env, finetuning=args.finetune) for _ in range(args.training_num)])
    test_envs = DummyVectorEnv([lambda: CrowdEnv(init_env, finetuning=args.finetune) for _ in range(args.test_num)])
...
if __name__ == "__main__":
    ...
    # should be fixed seed. see environment.py
