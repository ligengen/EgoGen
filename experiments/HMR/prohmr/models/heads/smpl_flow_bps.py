...
class SMPLFlowBPS(nn.Module):
    ...
    def __init__(self, bps_dim, cfg: CfgNode, contect_feats_dim=None):
        ...
        """
        super(SMPLFlowBPS, self).__init__()
        self.cfg = cfg
        self.npose = 6*(cfg.SMPL.NUM_BODY_JOINTS + 1)

        self.flow = ConditionalGlow(cfg.MODEL.FLOW.DIM+bps_dim, cfg.MODEL.FLOW.LAYER_HIDDEN_FEATURES,
    ...
    def log_prob(self, bps_encoding, smpl_params: Dict, feats: torch.Tensor) -> Tuple:
        """
        ...
        """
        # bps_encoding: [bs, 1, 24/66/6980]
        feats = feats.float()
        batch_size = feats.shape[0]
        samples = torch.cat((smpl_params['global_orient'], smpl_params['body_pose'], bps_encoding), dim=-1)  # [bs, 1, 144+24/66/6980]
    ...
    def forward(self, feats: torch.Tensor, num_samples: Optional[int] = None, z: Optional[torch.Tensor] = None) -> Tuple:
        ...
        if z is None:
            samples, log_prob, z = self.flow.sample_and_log_prob(num_samples, context=feats)  # samples: [bs, n_sample, 144+24/66/6980]
            z = z.reshape(batch_size, num_samples, -1)
        ...
        # print('max/min of pred_pose:', pred_pose.min().item(), pred_pose.max().item())
        ...
        pred_bps_encoding = pred_params[:, :, self.npose:]  # [bs, 1/num_sample, 24/66/6980]

        pred_betas, pred_cam = self.fc_head(pred_smpl_params, feats)
        pred_smpl_params['betas'] = pred_betas

        return pred_bps_encoding, pred_smpl_params, pred_cam, log_prob, z, pred_pose_6d
