...
def get_example(img_path: str, center_x: float, center_y: float,
                ...
                do_augment: bool, augm_config: CfgNode, fx, cam_cx, cam_cy,
                scene_pcd_verts, scene_pcd_colors,
    ...
    # img_patch_cv_no_aug, _ = generate_image_patch(cvimg,
    #                                            center_x_orig, center_y_orgin,
    #                                            width_orgin, height_orgin,
    #                                            patch_width, patch_height,
    #                                            False, 1.0, 0)
    # img_patch_cv_no_aug = img_patch_cv_no_aug[:, :, ::-1]
    # img_patch_no_aug = convert_cvimg_to_tensor(img_patch_cv_no_aug)
    # for n_c in range(img_channels):
    #     img_patch_no_aug[n_c, :, :] = np.clip(img_patch_no_aug[n_c, :, :] * color_scale[n_c], 0, 255)
    #     if mean is not None and std is not None:
    #         img_patch_no_aug[n_c, :, :] = (img_patch_no_aug[n_c, :, :] - mean[n_c]) / std[n_c]
    ...
    scene_pcd_verts_full_noauge = scene_pcd_verts.copy()  # [20000, 3]
    scene_pcd_verts_crop_noauge = scene_pcd_verts_full_noauge - cam_t_full + cam_t_crop
    scene_pcd_verts_crop_auge = scene_verts_3d_processing(scene_pcd_verts_crop_noauge, rot, do_flip=do_flip)
    ...
    if do_flip:
        cam_t_full[0, 0] = -cam_t_full[0, 0]  # todo: not exactly like this
        cam_t_crop[0, 0] = -cam_t_crop[0, 0]
    keypoints_3d_full_auge = keypoints_3d_crop_auge - cam_t_crop + cam_t_full

    scene_pcd_verts_full_auge = scene_pcd_verts_crop_auge - cam_t_crop + cam_t_full
    ...
    if do_augment:
        ...
        local_pelvis_joint = smpl_model(**body_params_dict_new).joints[0, 0].detach().numpy().squeeze()
        transl_full_auge = keypoints_3d_full_auge[0] - local_pelvis_joint
    ...
    return img_patch, keypoints_2d_crop, keypoints_2d_crop_vis_mask, \
           ...
           center_x_auge, center_y, cam_cx_auge, auge_scale, keypoints_2d_aug_orig, rotated_img, rotated_cvimg, scene_pcd_verts_full_auge
