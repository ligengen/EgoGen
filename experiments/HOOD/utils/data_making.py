...
def convert_amass_to_pkl(amass_seq_path, out_path, start=0, n_frames=None, n_inter_steps=0, target_fps=30,
    ...
    :return:
    """
    if "npz" in amass_seq_path:
        seq_dict = np.load(amass_seq_path)
        mocap_framerate = seq_dict['mocap_frame_rate']
    ...
    elif "ego" in amass_seq_path:
        print('ego')
        seq_dict = np.load(amass_seq_path, allow_pickle=True)
        transl = seq_dict['transl']
        betas = seq_dict['betas']
        body_pose = seq_dict['body_pose']
        global_orient = seq_dict['global_orient']

    else: # samp data
        import pickle
        with open(amass_seq_path, "rb") as f:
            seq_dict = pickle.load(f, encoding='latin1')
        f.close()
        ...
        sparcity = int(mocap_framerate / target_fps)
        transl = seq_dict['pose_est_trans'][start::sparcity]
        betas = seq_dict['shape_est_betas'][:10]
        poses = seq_dict['pose_est_fullposes'][start::sparcity]
