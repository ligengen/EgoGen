...
class CrowdEnv(gym.Env):
    def __init__(self, init_env, save_rollout=True, render=False):
        ...
        self.smplxparser_mp, self.feet_marker_idx, self.marker, self.vposer, self.man_id, self.name = init_env
        ...
        with torch.no_grad():
            ...
            if self.save_rollout:
                if terminated or truncated:
                    self.flag = True
                    # if r_goal > 0:
                    save_rollout_results(self.body_scene_data, self.outmps, "log/eval_results/crowd-4human", self.name + self.man_id)
    ...
    def reset(self):
        ...
        with torch.no_grad():
            ...
            self.floor = [[4,4], [4,-4], [-4,-4], [-4,4], [4,4]]
            ...
            distxy, dist, fea_wpath, fea_marker, fea_marker_h, points_local, local_map = self._get_feature(marker_seed, pelvis_loc,
                                                                                         R0, T0, wpath[-1:], self.body_scene_data)
    ...
    def _get_feature(self, Y_l, pel, R0, T0, pt_wpath, scene):
        ...
        if type(holes_multipoly)==shapely.geometry.multipolygon.MultiPolygon:
            for poly in holes_multipoly.geoms:
                holes.append(poly.exterior.coords[::-1])
        elif type(holes_multipoly)==shapely.geometry.polygon.Polygon:
            holes.append(holes_multipoly.exterior.coords[::])
        else:
            pdb.set_trace()

        # sort hole vertices in counter-clockwise
        """
        sorted_holes = []
        for hole in holes:
            polygon = hole[:-1]
            lowest = min(polygon, key = lambda x: (x[1], x[0]))
            sorted_hole = sorted(polygon, key=lambda x: math.atan2(x[1]-lowest[1], x[0]-lowest[0]) + 2 * np.pi)
            sorted_hole.append(sorted_hole[0])
            sorted_holes.append(sorted_hole)

        self.scene_poly = Polygon(self.floor, sorted_holes)
        """
        self.scene_poly = Polygon(self.floor, holes)
        if not self.scene_poly.is_valid:
            pdb.set_trace()

        points_local, map = self._get_dynamic_map(R0, T0, self.scene_poly) 
