...
def create_loader(mcfg: Config):
    ...
    # GEN: TODO
    smpl_model = smplx.SMPLX(smpl_model_path, batch_size=mcfg.batch_size)
...
class GarmentBuilder:
    ...
    def make_cloth_verts(self, body_pose: np.ndarray, global_orient: np.ndarray, transl: np.ndarray, betas: np.ndarray,
        ...
        full_pose = torch.cat([full_pose, torch.zeros((full_pose.shape[0], 99))], dim=1)
...
class BodyBuilder:
    ...
    def __init__(self, mcfg: Config, smpl_model: SMPLX, obstacle_dict: dict):
        """
        :param mcfg: Config
        :param smpl_model:
...
class Loader:
    ...
    def __init__(self, mcfg: Config, garments_dict: dict, smpl_model: SMPLX,
