...
# BETAS = torch.FloatTensor([[-1.0430452 , -0.82611334, -0.4263743 ,  0.20855242, -0.01421866, 0.08093569, -0.00256519,  0.11069646, -0.31370017, -0.09880832]])
BETAS = torch.zeros((1, 10))
A_POSE = torch.zeros((1, 165))
A_POSE[0, 50] = -0.7853981633974483
A_POSE[0, 53] = 0.7853981633974483
...
def make_lbs_dict(obj_file, smpl_file, n_samples=0):
    ...
    smpl_model = smplx.SMPLX(smpl_file)
    # TODO: need to change betas for each bedlam
    smplx_v_rest_pose = smpl_model(betas=BETAS, body_pose=A_POSE[:, 3:66]).vertices[0].detach().cpu().numpy()
    ...
    if n_samples == 0:
        ...
        garment_shapedirs = smpl_model.shapedirs[nn_inds].numpy()
        garment_posedirs = smpl_model.posedirs.reshape(486, -1, 3)[:, nn_inds].reshape(486, -1).numpy()
        garment_lbs_weights = smpl_model.lbs_weights[nn_inds].numpy()
    else:
        garment_shapedirs = 0
        garment_posedirs = 0
