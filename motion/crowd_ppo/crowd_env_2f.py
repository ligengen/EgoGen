...
from crowd_ppo.utils import calc_sdf, save_rollout_results
import pdb
import json
import warnings
...
class CrowdEnv(gym.Env):
    def __init__(self, init_env, save_rollout=True, render=False, finetuning=False):
        ...
        self.smplxparser_mp, self.feet_marker_idx, self.marker, self.vposer, self.scene_sdf = init_env
        ...
        with open("data/smplx_vert_segmentation.json", "r") as f:
            smplx_vids = json.load(f)
        self.feet_vids = []
        for parts in smplx_vids.keys():
            if parts in ['leftToeBase', 'rightToeBase', 'leftFoot', 'rightFoot']:
                self.feet_vids.extend(smplx_vids[parts])
        self.feet_vids = list(set(self.feet_vids))
    ...
    def step(self, action_z):
        ...
        with torch.no_grad():
            ...
            # sdf penetration reward TODO: is pred_vertices same with pred_marker_b? one blended one not. use pred_param to calc reward, correct?
            ...
            # remove feet vertices pene detection for finetuning (penetration will terminate)
            # if self.finetuning:
            #     vertices_w = vertices_w[:, :, self.no_feet_vids, :]
            sdf_values = calc_sdf(vertices_w.reshape(nb * nt, -1, 3), self.scene_sdf).reshape(nb, -1) # [b, t*p]
            # sdf penetration visualization
            self.pene_idx = torch.where(sdf_values.reshape(nb, nt, -1).lt(0.0)[0, -2, :])[0].detach().cpu().numpy()
            sdf_values = sdf_values.reshape(nb, nt, -1)
            sdf_values[:, :, self.feet_vids] = 0. # do not consider feet floor pene
            # negative_values = sdf_values * (sdf_values < 0)
            # r_pene = torch.exp(negative_values.sum(dim=(1, 2)) / nt)[0]
            num_inside = sdf_values.lt(0.0).sum(dim=(1,2))[0] / nt / 10
            num_inside_max = sdf_values.lt(0.0).sum(dim=-1).max(dim=-1).values[0]
            penetration = False if num_inside_max < 40 else True 
            r_pene = torch.exp(-num_inside)
            ...
            distxy, dist, fea_wpath, fea_marker, fea_marker_h = self._get_feature(marker_seed, pel_loc_seed, \
                                                                                    self.R0, self.T0, \
            ...
            # set different penetration weight 
            if self.finetuning:
                weight_pene = 0.1
            else:
                weight_pene = 1.
            ...
            if self.save_rollout:
                if terminated or truncated:
                    self.flag = True
                    # if r_goal > 0:
                    save_rollout_results(self.body_scene_data, self.outmps, './log/eval_results/')
    ...
    def reset(self, seed=None, options=None):
        ...
        with torch.no_grad():
            self.outmps = []
            while True:
                body_scene_data = self.scene_sampler.next_body(visualize=False, use_zero_pose=False,\
                ...
                seed_output = self.smplxparser_2frame.forward_smplx(
                    betas=prev_betas,
                    gender=gender,
                ...
                marker_seed = seed_output.vertices[:,self.marker,:].reshape(nb, nt, -1)
                ...
                pred_joints_all = seed_output.joints[:,:,:].reshape(nb, nt, -1, 3)
                # pelvis_loc = pelvis_loc.reshape(nb, nt, -1)
                pelvis_loc = pred_joints_all[:, :, 0]

                distxy, dist, fea_wpath, fea_marker, fea_marker_h = self._get_feature(marker_seed, pelvis_loc,
                                                                                             R0, T0, wpath[-1:], body_scene_data)
                # use sdf to check initial collision
                pred_vertices = seed_output.vertices[:,:,:].reshape((nb, nt, -1, 3))
                vertices_w = torch.einsum("bij,btpj->btpi", R0, pred_vertices) + T0[:, None, :, :]
                sdf_values = calc_sdf(vertices_w.reshape(nb * nt, -1, 3), self.scene_sdf).reshape(nb, -1) # [b, t*p]
                # sdf penetration visualization
                self.pene_idx = torch.where(sdf_values.reshape(nb, nt, -1).lt(0.0)[0, -2, :])[0].detach().cpu().numpy()
                sdf_values = sdf_values.reshape(nb, nt, -1)
                ...
                # negative_values = sdf_values * (sdf_values < 0)
                # r_pene = torch.exp(negative_values.sum(dim=(1, 2)) / nt)[0]
                num_inside = sdf_values.lt(0.0).sum(dim=(1,2))
                if num_inside[0] == 0:
    ...
    def render(self):
        ...
        # visualize sdf penetration
        init_body_mesh.visual.vertex_colors[self.pene_idx] = np.array([200, 100, 100, 255])
    ...
    def _calc_egosensing(self, joint):
        ...
        # joint: [t=2, J=127, 3]
        # TODO: change to 2f sensing like markers
